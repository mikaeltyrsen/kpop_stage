...
def api_status() -> Any:
    ...
    dmx_levels: List[int] = []
    output = getattr(dmx_manager, "output", None)
    if output and hasattr(output, "get_levels"):
        try:
            levels_iter = output.get_levels()
            raw_levels = levels_iter if isinstance(levels_iter, list) else list(levels_iter)
            dmx_levels = []
            for value in raw_levels:
                ...
                try:
                    ...
                    dmx_levels.append(max(0, min(255, numeric)))
        except Exception:  # pragma: no cover - defensive logging
            LOGGER.exception("Unable to capture DMX levels for status response")
            dmx_levels = []

    payload["dmx"] = {"levels": dmx_levels}
